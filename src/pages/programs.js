...
function renderPage() {
  ...
  const p = siteContent[currentLang].programs;
  const isMr = currentLang === 'mr';
  const isHi = currentLang === 'hi';

  app.innerHTML = `
    ${createNavbar(siteContent, currentLang, 'programs')}
    
    <main style="flex: 1;">
      
      <!-- Programs Hero Header with Breadcrumbs -->
        ...
        <div class="container text-center" style="max-width: 850px; margin: 0 auto;">
          <div style="margin-bottom: 1rem;">
            ...
            <span style="color: var(--foreground-muted); font-size: 1.05rem; font-weight: 700;">
              ${isMr ? 'कार्यक्रम आणि उपक्रम' : isHi ? 'कार्यक्रम एवं गतिविधियाँ' : 'Programs & Activities'}
            </span>
          </div>
      ...
      <!-- Programs Grid -->
      ${createProgramsSection(siteContent, currentLang, false)}
    ...
    ${createNavigationDrawer(siteContent, currentLang, 'programs')}
  `;
...
function attachPageListeners() {
  try {
    ...
    console.warn('Programs page listener setup warning:', err);
  }

  // Category Filters & Native IntersectionObserver
  const programFilterBtns = document.querySelectorAll('.program-filter-btn');
  const programCards = Array.from(document.querySelectorAll('.program-card-step, .program-card'));
  ...
  if (lightboxPrevBtn) lightboxPrevBtn.addEventListener('click', () => updateLightbox(activeLightboxIndex - 1));
  if (lightboxNextBtn) lightboxNextBtn.addEventListener('click', () => updateLightbox(activeLightboxIndex + 1));
  if (lightboxModal) {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (modal && modal.classList.contains('open')) {
      ...
      if (e.key === 'ArrowLeft') updateLightbox(activeLightboxIndex - 1);
      if (e.key === 'ArrowRight') updateLightbox(activeLightboxIndex + 1);
    }
  });
